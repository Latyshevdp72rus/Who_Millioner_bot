...
async def cancel_handler(message: Message, state: FSMContext):
    await state.finish()
    await message.answer(text='Ещё увидимся', reply_markup=ReplyKeyboardRemove())
...
async def get_menu(message: Message, state: FSMContext):
    if message.text == '<НОВАЯ  ИГРА>':
        ...
        return await message.answer('Не флудите!')
...
async def get_menus(message: Message, state: FSMContext):
    ...
    if message.text == '50 на 50' and user_obj.fifth:
        ...
        await bot.edit_message_reply_markup(user_obj.user_id, user_obj.cur_msg,
                                            reply_markup=render_fifth(user_obj.cur_quest, user_obj.step))
...
async def blabla(call: CallbackQuery, state: FSMContext):
    ...
    if call.data.split(':')[1] != str(user_obj.step):
        return await call.answer('Упс, не попал!')
    ...
    if check_answer(user_obj, call.message.text, answer) and user_obj.step == 10:
        ...
        await bot.send_message(chat_id=call.message.chat.id, text='Упс, вы  проиграли.')
        await bot.send_message(chat_id=call.message.chat.id, text='Ваш выигрыш: ' + str(user_obj.balance) + ' руб',
                               reply_markup=ReplyKeyboardRemove())
        return await call.answer()